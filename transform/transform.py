...
def ras2lps(pm, verbose=False):
    ...
    conv = np.eye(4) #lps to ras matrix
    conv[0, 0] = -1
    conv[1, 1] = -1
    ...
    A_new = tuple([str(y) for y in lps[:3, :3].ravel()])
    t_new = tuple([str(y) for y 
        in np.array([lps[0, 3], lps[1, 3], lps[2, 3]])])
    if verbose:
        print("In LPS: " + str(A_new + t_new))
    pm['TransformParameters'] = A_new + t_new
